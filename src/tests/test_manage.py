...
def run_generate_divisions(
    ...
    for i, division in enumerate(divisions):
        print(f"expected_teams[{i}]: {expected_teams[i]}")
        print(f"divisions[{i}]: {division[0]['team']} - {division[-1]['team']}")
        team_names = [team["team"] for team in division]
        for team in expected_teams[i]:
